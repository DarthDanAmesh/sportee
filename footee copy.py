...
class MatchRepository:
    ...
    def load_list(self, data: List[Tuple]) -> int:
        """
        Accepts:
          (date_str, home, away, home_score, away_score)
          (home, away, home_score, away_score)          ← date = 1900-01-01
        """
        matches: List[Match] = []
        for item in data:
            if len(item) == 5:
                raw_date, h, a, hs, as_ = item
                parsed = _parse_date(raw_date) if isinstance(raw_date, str) else (
                    raw_date if isinstance(raw_date, date) else date(1900, 1, 1)
                )
            elif len(item) == 4:
                h, a, hs, as_ = item
            ...
            matches.append(Match(
                date=parsed,
                home_team=TeamRegistry.normalize(str(h)),
                away_team=TeamRegistry.normalize(str(a)),
                home_score=int(hs),
                away_score=int(as_),
            ))
        return self.add_matches(matches)
...
if __name__ == "__main__":
    ...
    historical = [
        ...
        ("2025-10-11", "Athletic",              "Goiás",           1, 1),
        ("2024-06-24", "Goiás",                 "Athletic",        1, 2),
        ("2024-10-11", "Athletic",              "Goiás",           2, 1),
        ("2023-06-24", "Goiás",                 "Athletic",        1, 0),
    ]

    # ── future matches to predict (with odds) ──
    future = [
        #("Lazio",         "Napoli",          2.10, 3.30, 3.50),
        ("Athletic",              "Goiás",           2.15,3.1,3.5),
    ...
    # Force UTF-8 output with replacement for unsupported characters
